...
public class MainGame {
    ...
    private final int TARGET_COUNT_TO_SEND_TILES = 5;
    private boolean mIsVsMode = false;
    private int countToSendTiles;
    private OnGameEventListener mOnGameEventListener = null;

    public interface OnGameEventListener {
		public void onSendTiles();
    ...
    public void setIsVsMode(boolean isVsMode){
    	this.mIsVsMode = isVsMode;
    }
    ...
    public void setOnGameEventListener(OnGameEventListener listener){
		this.mOnGameEventListener = listener;
	} 
    ...
    public void newGame() {
        ...
        mView.resyncTime();
        Log.d("emma", "newGame call invalidate");
        mView.invalidate();
    }
    ...
    public void revertUndoState() {
        if (canUndo) {
            ...
            mView.refreshLastTime = true;
            Log.d("emma", "revertUndoState call invalidate");
            mView.invalidate();
        }
    }
    ...
    public void move (int direction) {
            ...
            for (int yy: traversalsY) {
                ...
                if (tile != null) {
                    ...
                    if (next != null && next.getValue() == tile.getValue() && next.getMergedFrom() == null) {
                        ...
                        if(mIsVsMode){
                        	countToSendTiles++;
                        	Log.d("emma", "countToSendTiles="+countToSendTiles);
                        	if(countToSendTiles >= TARGET_COUNT_TO_SEND_TILES){
                        		countToSendTiles = 0;
                        		if(mOnGameEventListener != null){
                        			Log.d("emma", "trigger Event onSendTiles()");
                        			mOnGameEventListener.onSendTiles();
                        		}
                        	}
                        }
                        ...
                        if (merged.getValue() >= winValue() && !gameWon()) {
                            ...
                            if(mIsVsMode){
                            	if(mOnGameEventListener != null){
                            		mOnGameEventListener.onGameWin();
                            	}
                            }
                            endGame();
                        }
        ...
        mView.resyncTime();
        Log.d("emma", "move call invalidate");
        mView.invalidate();
    }
    
    public void recieveRandomTile(){
    	try{
...
//        saveUndoState();
        ...
        mView.resyncTime();
        Log.d("emma", "recieveRandomTile call invalidate");
        mView.invalidate();
    }
    
    public void winInVSMode(){
    	if (!gameWon()) {
            gameState = gameState + GAME_WIN; // Set win state
            endGame();
            mView.resyncTime();
            Log.d("emma", "winInVSMode call invalidate");
            mView.invalidate();
        }
    }
    
    public void loseInVSMode(){
    	if (!gameWon()) {
    		gameState = GAME_LOST;
            endGame();
            mView.resyncTime();
            Log.d("emma", "loseInVSMode call invalidate");
            mView.invalidate();
        }
    }
    ...
    private void checkLose() {
        if (!movesAvailable() && !gameWon()) {
            ...
            if(mIsVsMode){
            	if(mOnGameEventListener != null){
            		mOnGameEventListener.onGameLose();
            	}
            }
            endGame();
    ...
    public void setEndlessMode() {
        ...
        Log.d("emma", "setEndlessMode call invalidate");
        mView.invalidate();
        mView.refreshLastTime = true;
    }
